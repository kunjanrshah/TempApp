...
//	public static boolean isDebug=false;
	...
	public String PostConnection(String strUrl,ArrayList<NameValuePair> alstNameValuePair) {
		...
		try {
		  ...
		  Log.i("CommonClass","Result :"+strResult);
		 return strResult;
	}
    ...
    public static int GetDays(String Start_Date,String End_Date){
        try{
                SimpleDateFormat mSimpleDateFormat = new SimpleDateFormat("dd-MM-yyyy k:mm");
//                SimpleDateFormat mSimpleDateFormat = new SimpleDateFormat("dd-MM-yyyy h:mm aa");
                Date mStartDate = mSimpleDateFormat.parse(Start_Date);
                Date mEndDate = mSimpleDateFormat.parse(End_Date);
                 long diffInMis = mEndDate.getTime() - mStartDate.getTime();
                 long diffInSec = TimeUnit.MILLISECONDS.toSeconds(diffInMis);
                 int days =(int) diffInSec / (24 * 60 * 60);
                 return days;
        }catch(Exception e){
                e.printStackTrace();
                return 0;
